...
public class PerfumeServiceImpl implements PerfumeService {
    ...
    public PerfumeDto details(int id) {
        ...
        for (PerfumePrfNotes note : notes) {
            PerfumePrfNotesDto noteDto = new PerfumePrfNotesDto(note.getNote().getName(), note.getType());
        ...
        for (Review review : reviews) {
            ...
            String authorUsername = review.getUser().getUsername();
            ReviewDto dto = new ReviewDto(userName, review.getTitle(), review.getDescription(),
                    review.getVote(), review.getSeasonality(), review.isGender(),
                    review.getSillage(), review.getLongevity());
            dto.setId(review.getId());
            dto.setAuthorUsername(authorUsername);
            reviewDto.add(dto);
        }
        PerfumeDto perfumeDto = new PerfumeDto(perfume.getName(), perfume.getBrand().getName(),
                perfume.getDescription(), notesDto, parfumers, reviewDto);
    ...
    @Override
    public Page<PerfumeDto> findAllPerfumes(Pageable pageable) {
        logger.debug("Fetching all perfumes - page: {}, size: {}", pageable.getPageNumber(), pageable.getPageSize());
        Page<PerfumeDto> result = perfumeRepository.findAll(pageable)
                .map(p -> {
                    PerfumeDto dto = new PerfumeDto(p.getName(), p.getBrand().getName(), null, null, null, null);
                    dto.setId(p.getId());
                    dto.setImageUrl(p.getImageUrl());
                    return dto;
                });
        logger.info("Returning page {}/{} ({} total perfumes)",
                result.getNumber() + 1, result.getTotalPages(), result.getTotalElements());
        return result;
    }

    @Override
    public Page<PerfumeDto> findPerfumesByFilters(String name, String parfumer, String brand, String note, Pageable pageable) {
        logger.debug("Filtering perfumes - name: {}, parfumer: {}, brand: {}, note: {}", name, parfumer, brand, note);
        return perfumeRepository.findByFilters(name, parfumer, brand, note, pageable)
                .map(p -> {
                    PerfumeDto dto = new PerfumeDto(p.getName(), p.getBrand().getName(), null, null, null, null);
                    dto.setId(p.getId());
                    dto.setImageUrl(p.getImageUrl());
