...
public class ParfumerServiceImpl implements ParfumerService {
    ...
    public ParfumerDto create(ParfumerDto p) {
        ...
        Parfumer parfumerNew = new Parfumer(
                ...
                p.getDescription(),
                p.getNationality());
        parfumerNew.setImageUrl(p.getImageUrl());
    ...
    public ParfumerDto updateparfumer(int id, ParfumerDto p) {
        ...
        if (p.getNationality() != null) {
            parfumer.setNationality(p.getNationality());
        }
        if (p.getImageUrl() != null) {
            parfumer.setImageUrl(p.getImageUrl());
        }
    ...
    public ParfumerDto deleteById(int id) {
        ...
        long perfumeCount = parfumerRepository.countPerfumesByParfumerId(id);
        if (perfumeCount > 0) {
            logger.warn("Cannot delete parfumer {}: still referenced by {} perfumes", id, perfumeCount);
            throw new EntityInUseException(
                "Non è possibile eliminare questo profumiere: è associato a " + perfumeCount +
                " profum" + (perfumeCount == 1 ? "o" : "i") + ". Rimuovi prima l'associazione dai profumi collegati."
            );
        }
