...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(NoNameShelfException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public ErrorResponse handleNoNameShelf(NoNameShelfException e, HttpServletRequest request) {
        logger.warn("Blank shelf name on {}: {}", request.getRequestURI(), e.getMessage());

        ErrorResponse response = new ErrorResponse();
        response.setStatus(400);
        response.setTimestamp(LocalDateTime.now());
        response.setError("Invalid shelf name");
    ...
    public ErrorResponse handleInvalidCredentials(InvalidCredentialsException e, HttpServletRequest request) {
        logger.warn("Invalid credentials on {}: {}", request.getRequestURI(), e.getMessage());
    ...
    @ResponseBody
    public ErrorResponse handleDuplicateReview(DuplicateReviewException e, HttpServletRequest request) {
        logger.info("Duplicate review on {}: {}", request.getRequestURI(), e.getMessage());
    ...
    @ResponseStatus(HttpStatus.CONFLICT)
    @ResponseBody
    public ErrorResponse handleUsernameAlreadyTaken(UsernameAlreadyTakenException e, HttpServletRequest request) {
        logger.warn("Username already taken on {}: {}", request.getRequestURI(), e.getMessage());

        ErrorResponse response = new ErrorResponse();
        response.setStatus(409);
        response.setTimestamp(LocalDateTime.now());
        response.setError("Conflict — username unavailable");
        response.setMessage(e.getMessage());
        response.setPath(request.getRequestURI());
        return response;
    }
